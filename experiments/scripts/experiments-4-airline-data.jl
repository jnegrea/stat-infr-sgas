...
## Example 4.7: Iteravg test
experiment_name = "experiments-4-7-airline-data-iteravg-test"
output_path_4_7 = output_path_4*experiment_name*"/"
isdir(output_path_4_7) || mkdir(output_path_4_7)

samplesize = 100000
w=1/2


algos = [:vanillaSGDPowHalf,
:vanillaSGDPowTwoThird,
:vanillaSGDPowThird,
:preconSGLD,
:preconSGD
]
...
# airlineInit(samplesize, output_path_4_7*"model_init", source_data_file; trueparmfile=output_path_4*"airline-data-trueparm.jld2" , initparm=false, mc_size=false, mleparmtype=:glm)
# modelRun(output_path_4_7*"model_init.jld2", output_path_4_7*"model_run.jld2",
#     algos, samplesize, batchsizes, w, numstepss; multithread=true, ismmap_run=true, keepperiods=keepperiods)
...
# modelPlot(output_path_4_7*"model_run.jld2", output_path_4_7*experiment_name*"-fig",
    # layers, (-25,25), (-25,25), nameattr= false, levels=10, localscale=false)

layers = layerer([:mixture, :preconSGLD, :trueparm, :noaxes, :nolegend])

layers = layerer(layers, [:vanillaSGDPowHalf, :sandwich, :iteravg], drop=[:mixture, :preconSGLD])
layers = layerer(layers, [:vanillaSGDPowThird], drop=[:vanillaSGDPowHalf])
layers = layerer(layers, [:vanillaSGDPowTwoThird], drop=[:vanillaSGDPowThird])
layers = layerer(layers, [:preconSGD], drop=[:vanillaSGDPowTwoThird, :iteravg])

modelPlot(output_path_4_7*"model_run.jld2", output_path_4_7*experiment_name*"-fig",
    layers, (-40,40), (-40,40), nameattr= false, levels=10, localscale=false)

## Example 4.8: Iteravg test
experiment_name = "experiments-4-8-airline-data-iteravg-test"
output_path_4_8 = output_path_4*experiment_name*"/"
isdir(output_path_4_8) || mkdir(output_path_4_8)
...
airlineInit(samplesize, output_path_4_8*"model_init", source_data_file; trueparmfile=output_path_4*"airline-data-trueparm.jld2" , initparm=false, mc_size=false, mleparmtype=:glm)
modelRun(output_path_4_8*"model_init.jld2", output_path_4_8*"model_run.jld2",
    algos, samplesize, batchsizes, w, numstepss; multithread=true, ismmap_run=true, keepperiods=keepperiods)
...
# modelPlot(output_path_4_8*"model_run.jld2", output_path_4_8*experiment_name*"-fig",
#     layers, (-25,25), (-25,25), nameattr= false, levels=10, localscale=false)
